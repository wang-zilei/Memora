...
- `app/web/src/App.tsx` 中的展示结构、className、品牌文字和非业务视觉容器。
- `app/web/src/index.css` 中的视觉系统、布局、颜色、字体、间距、边框、阴影与交互状态。
...
**目录语义**：`demo/` 仅保留为早期本地实验目录，不再作为正式客户端/插件源码提交入口。

**调整**：正式前端从 `demo/web` 迁移到 `app/web`，正式浏览器插件从 `demo/extension` 迁移到 `extension`；Tauri 配置、GitHub Release 工作流和 UI guardrails 同步指向新目录。未修改产品功能、API、数据库或抓取链路语义。
