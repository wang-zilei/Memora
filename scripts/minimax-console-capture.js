/*
 * MiniMax conversation capture probe.
 *
 * Usage:
 * 1. Open a MiniMax/Hailuo conversation page, for example https://chat.minimax.io/... or https://hailuoai.com/...
 * 2. Open DevTools Console.
 * 3. Paste this whole script and press Enter.
 * 4. Inspect window.minimaxCaptureResult.
 *
...
(async () => {
  const CONFIG = {
    platform: 'minimax',
    allowedHosts: ['chat.minimax.io', 'www.minimax.io', 'hailuoai.com', 'www.hailuoai.com'],
    resultName: 'minimaxCaptureResult',
    maxScrollPasses: 8,
    settleMs: 450,
    copyToClipboard: true,
    selectors: {
      turns: ['[data-testid*="message"]', '[class*="message"]', '[class*="chat"] [class*="bubble"]', '[class*="conversation"] [class*="item"]', 'main article'],
  ...
  function createVisibleDomProbe(config) {
    ...
    const junkPatterns = [/^(新建|历史|设置|登录|注册|升级|会员|分享|复制|重新生成|停止生成|发送|展开|收起|编辑|删除)$/, /^(minimax|hailuo|海螺)$/i, /^(new chat|history|settings|login|upgrade|share|copy|retry|regenerate|stop)$/i];
    ...
    return async function run() {
      ...
      const result = {
        ...
        notes: ['Visible/loaded DOM probe; internal MiniMax API is not encoded in this script.'],
      };
