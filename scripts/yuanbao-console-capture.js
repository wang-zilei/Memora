/*
 * Tencent Yuanbao conversation capture probe.
 *
 * Usage:
 * 1. Open a Yuanbao conversation page, for example https://yuanbao.tencent.com/chat/...
 * 2. Open DevTools Console.
 * 3. Paste this whole script and press Enter.
 * 4. Inspect window.yuanbaoCaptureResult.
 *
...
(async () => {
  const CONFIG = {
    ...
    resultName: 'yuanbaoCaptureResult',
    maxScrollPasses: 8,
    settleMs: 450,
  ...
  function createVisibleDomProbe(config) {
    ...
    return async function run() {
      ...
      const result = {
        ...
        notes: ['Visible/loaded DOM probe; internal Yuanbao API is not encoded in this script.'],
      };
    ...
    async function scrollPageForLazyContent() {
      for (const target of getScrollTargets()) {
        for (let pass = 0; pass < config.maxScrollPasses; pass += 1) {
          ...
          await sleep(config.settleMs);
          if (Math.abs(getScrollTop(target) - before) < 2) break;
        }
      }
    }
    ...
    function collectTurnCandidates() {
      ...
      return (turnElements.length ? turnElements.map((element) => ({ element, role: inferRole(element) })) : roleElements)
    ...
    function inferRole(element) {
      ...
      if (/assistant|agent|bot|answer|ai|markdown|response/.test(own)) return 'assistant';
